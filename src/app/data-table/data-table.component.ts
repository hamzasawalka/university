...
export class DataTableComponent implements OnInit {
	...
	async ngOnInit() {
		this.students = await this.qs.getStudents().then(students => students);
		...
		if (!!this.students) {
			this.students.forEach(e => {
				...
				this.createTable(this.students);
			});
		} 
	...
	toArrayWithNested(obj, property) {
		return this.toArray(obj).map((e) => {
			e[property] = this.toArray(e[property])
			return e;
		});
	}
